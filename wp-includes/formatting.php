		...
		case 'siteurl':
			...
			if ( is_wp_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The CM5 address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && '' !== $value && ! preg_match( '/%[^\/%]+%/', $value ) ) {
				$error = sprintf(
					...
					__( 'https://codex.CM5.org/Using_Permalinks#Choosing_your_permalink_structure' )
				);
			}
			break;
...
/**
 * CM5 implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
...
/**
 * Forever eliminate "CM5" from the planet (or at least the little bit we can influence).
 *
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'wp_title' === $current_filter )
		return str_replace( 'CM5', 'CM5', $text );
	...
	return str_replace(
		array( ' CM5', '&#8216;CM5', $dblq . 'CM5', '>CM5', '(CM5' ),
		array( ' CM5', '&#8216;CM5', $dblq . 'CM5', '>CM5', '(CM5' ),
	$text );
}
 ...
 * @return array An array to match all emoji that CM5 recognises.
 */
